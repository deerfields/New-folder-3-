...
export class WorkPermitService {
  ...
  async approveWorkPermit(id: string, approvedBy: string, comments?: string): Promise<WorkPermit | null> {
    ...
    // تولید PDF و QR Code پس از تایید
    const pdfPath = await this.generateWorkPermitPDF(id);
    permit.documents = {
      ...(permit.documents || {}),
      pdf: pdfPath
    }
    await this.workPermitRepository.save(permit)
    return permit
  }
  ...
  async generateWorkPermitPDF(permitId: string): Promise<string> {
    ...
    // عنوان
    doc.fontSize(18).text('Work Permit', { align: 'center' });
    doc.moveDown();
    ...
    doc.text(`Start Date: ${permit.startDate}`);
    doc.text(`End Date: ${permit.endDate}`);
    doc.text(`Description: ${permit.workDescription}`);
    doc.moveDown();

    // QR Code (لینک بررسی وضعیت مجوز)
    const qrData = `https://mallos.com/permit/verify/${permit.id}`;
    const qrImage = await QRCode.toDataURL(qrData);
    const qrBuffer = Buffer.from(qrImage.split(",")[1], 'base64');
    doc.image(qrBuffer, { fit: [100, 100], align: 'center' });
    doc.text('Scan QR code to verify permit', { align: 'center' });
